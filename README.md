...
The browser creates the downloadable WebM video locally with Cloudy narration and on-screen captions. The export panel also provides SRT and WebVTT caption files plus a plain-text transcript, so published video can include a selectable captions track and an accessible text alternative. On first export, it downloads and caches a browser-local female voice model; later exports reuse that model automatically. No screen-sharing permission or tab-audio selection is required. Keep the tab active until the real-time render finishes.
